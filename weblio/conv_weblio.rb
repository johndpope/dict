...
in_file = File::open( "weblio_data.csv" )

# 出力ファイル
out_file = File::open( "weblio.csv", 'w' )
...
while line = in_file.gets
  ...
  # 行出力 
  out_file.puts "#{word},0,0,#{cost},名詞,一般,*,*,*,*,#{word},*,*,weblio"

  # 全角も登録
#  zen_word=NKF.nkf( '-w',word.tr('0-9','０-９').tr('a-z','ａ-ｚ').tr('A-Z','Ａ-Ｚ'))
#  if zen_word != word
#    out_file.puts "#{zen_word},0,0,#{cost},名詞,一般,*,*,*,*,#{zen_word},*,*,weblio"
#  end

  # 英字の場合は、小文字統一、大文字統一も出力しておく
#  if word != word.downcase
#    out_file.puts "#{word.downcase},0,0,#{cost},名詞,一般,*,*,*,*,#{word},*,*,weblio"
#  end
#  if word != word.upcase
#    out_file.puts "#{word.upcase},0,0,#{cost},名詞,一般,*,*,*,*,#{word},*,*,weblio"
#  end
